// Configuração do Firebase (desabilitada temporariamente)
...
function initializeSidebar() {
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            console.log('🎯 Menu clicado:', view);
            
            if (view === 'presidencia') {
                // Resetar navegação hierárquica
...
// Renderizar view de departamento específico
...
function renderPresidenciaView() {
    ...
    const title = document.createElement('h2');
    title.textContent = 'Presidência - Diretores';
    title.style.cssText = `
        text-align: center;
        margin: 0 0 30px 0;
        font-size: 28px;
...
function goBack() {
    if (navigationHistory.length === 0) {
        // Se não há histórico, voltar para presidência
        currentHierarchyLevel = 'presidencia';
        currentSelectedPerson = null;
        navigationHistory = [];
        renderPresidenciaView();
